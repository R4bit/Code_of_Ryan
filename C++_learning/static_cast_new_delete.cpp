...
将 malloc 的返回值^直接^转换为 int* 类型的指针，这是不安全的做法，因为 malloc 返回的指针类型是 void*，它不具有类型安全的属性，可能导致程序错误或崩溃。

第二行代码 pc = static_cast<int*>(static_cast<void*>(malloc(sizeof(int)))); 
^先^将 malloc 的返回值转换为 void* 类型的指针，再将其转换为 int* 类型的指针。这样做的好处是显式地将 void* 类型的指针转换为目标类型的指针，可以确保类型安全，并减少潜在的错误。
